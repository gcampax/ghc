...
compactGetFirstBlock buffer = IO (\s -> case compactGetFirstBlock# buffer s of
                                     (# s', addr, size #) -> (# s', (Ptr addr, W# size) #) )
compactGetNextBlock :: Compact# -> Addr# -> IO (Ptr a, Word)
compactGetNextBlock buffer block = IO (\s -> case compactGetNextBlock# buffer block s of
                                          (# s', addr, size #) -> (# s', (Ptr addr, W# size) #) )
...
withSerializedCompact :: NFData c => Compact a -> (SerializedCompact a -> IO c) -> IO c
...
importCompact (SerializedCompact ((_, W# firstSize):otherBlocks) (Ptr rootAddr)) filler =
