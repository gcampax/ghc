...
typedef struct StgCompactNFData_ {
    ...
    StgWord                totalW; // for proper accounting in evac, includes
                                   // slop, and removes the first block in larger
                                   // than megablock allocation
                                   // essentially meaningless, but if we got it
                                   // wrong sanity would complain loudly
    StgWord                totalDataW; // for stats/profiling only, it's the full amount
                                       // of memory used by this compact, including
                                       // the portions not yet used
